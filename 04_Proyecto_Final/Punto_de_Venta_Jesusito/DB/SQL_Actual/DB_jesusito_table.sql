...
create table cTipoEstado(
	cvTipoEstado varchar(3) not null,
    dsTipoEstado enum('Entregado','EnCamino','Retrasado','Cancelado') not null,
    primary key(cvTipoEstado)
);
...
create table pedidos(
    ...
    foreign key(cvTipoEstado) references cTipoEstado(cvTipoEstado),
    foreign key(idProveedor) references persona(idPersona)
);
